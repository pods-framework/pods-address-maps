	...
	class PodsField_Address_Map extends PodsField {
		...
		public function options () {
			$options = array(
				...
				'address_map_address_options' => array(
					'label' => __( 'Address Options', 'pods' ),
                    'depends-on' => array( 'address_map_type' => 'address' ),
					'group' => array(
						'address_map_address_line_1' => array(
							'label' => __( 'Enable Address Line 1', 'pods' ),
							'default' => 1,
							'type' => 'boolean'
						),
                        'address_map_address_line_2' => array(
                            'label' => __( 'Enable Address Line 2', 'pods' ),
                            'default' => 0,
                            'type' => 'boolean'
                        ),
                        'address_map_address_city' => array(
                            'label' => __( 'Enable City', 'pods' ),
                            'default' => 1,
                            'type' => 'boolean'
                        ),
                        'address_map_address_state' => array(
                            'label' => __( 'Enable State / Province', 'pods' ),
                            'default' => 1,
                            'type' => 'boolean',
                            'dependency' => true
                        ),
                        'address_map_address_country' => array(
                ...
                'address_map_address_state_input' => array(
                    'label' => __( 'State Input Type', 'pods' ),
                    'depends-on' => array( 'address_map_address_state' => true ),
                    'default' => 'text',
                    'type' => 'pick',
                    'data' => array(
                        'text' => __( 'Freeform Text', 'pods' ),
				...
				'address_map_address_country_input' => array(
					'label' => __( 'Country Input Type', 'pods' ),
					'depends-on' => array( 'address_map_address_country' => true ),
					'default' => 'text',
                    'type' => 'pick',
                    'data' => array(
                        'text' => __( 'Freeform Text', 'pods' ),
                ...
                'address_map_autocorrect' => array(
                    'label' => __( 'Autocorrect Address during save', 'pods' ),
                    'depends-on' => array( 'address_map_display_type' => array( 'single', 'multi' ) ),
                    'default' => 0,
                    'type' => 'boolean'
                ),
                'address_map_show_map_input' => array(
                    'label' => __( 'Show Map below Input', 'pods' ),
                    'default' => 0,
                    'type' => 'boolean'
                ),
                'address_map_display_type' => array(
                ...
                'address_map_style' => array(
                    ...
                    'depends-on' => array( 'address_map_display_type' => array( 'map', 'address-map' ) ),
                    'default' => pods_var( 'address_map_style', self::$component_options, 'static', null, true ),
                    'type' => 'pick',
                    'data' => array(
                ...
                'address_map_type_of_map' => array(
                    ...
                    'depends-on' => array( 'address_map_display_type' => array( 'map', 'address-map' ) ),
                    'default' => pods_var( 'address_map_type', self::$component_options, 'roadmap', null, true ),
                    'type' => 'pick',
                    'data' => array(
                ...
                'address_map_zoom' => array(
                    ...
                    'default' => pods_var( 'address_map_zoom', self::$component_options, 12, null, true ),
                    'type' => 'number',
                    'options' => array(
                        'number_decimals' => 0,
                        'number_max_length' => 2
                ...
                'address_map_marker' => array(
                    ...
                    'depends-on' => array( 'address_map_display_type' => array( 'map', 'address-map' ) ),
                    'default' => pods_var( 'address_map_marker', self::$component_options ),
                    'type' => 'file',
                    'options' => array(
		...
		public function input ( $name, $value = null, $options = null, $pod = null, $id = null ) {
            ...
            if ( 'address' == pods_var( 'address_map_type', $options ) )
                $field = PODS_DIR . 'ui/fields/address.php';
            elseif ( 'lat-long' == pods_var( 'address_map_type', $options ) )
                $field = PODS_DIR . 'ui/fields/lat-long.php';

            pods_view( $field, compact( array_keys( get_defined_vars() ) ) );

            if ( 1 == pods_var( 'address_map_show_map_input', $options ) )
			    pods_view( self::$file_path . 'ui/map.php', compact( array_keys( get_defined_vars() ) ) );
		}
        ...
        public static function geocode_address ( $address ) {
            ...
            if ( !empty( $address ) ) {
                ...
                if ( 'google' == self::$options[ 'provider' ] ) {
                    $args = array(
                        'address' => $address,
                        'sensor' => 'false'
                    );

                    if ( !empty( self::$options[ 'google_client_id' ] ) )
                        $args[ 'client' ] = self::$options[ 'google_client_id' ];
